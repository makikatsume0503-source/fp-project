...
- 数字・統計を参照した場合: 文中に「（参照：<a href="URL" target="_blank" rel="noopener">発行機関名｜資料名</a>）」のようにURL付きリンクで明記
- SVGキャプションで参照元がある場合: 「〇〇（参照：<a href="URL" target="_blank" rel="noopener">発行機関名｜資料名</a>　をもとに筆者作成）」と記載
- 図・表・画像を転載・直接引用した場合: キャプションに「出典：<a href="URL" target="_blank" rel="noopener">発行機関名｜資料名</a>」を明記
- 筆者が独自に作成した図解: キャプションに「筆者作成」と明記（URLなし）
- 情報の鮮度を明示（「〇〇年〇月時点」）
- URLは必ず実在する官公庁ページのURLを使用すること"""
